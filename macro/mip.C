...
void loadLibsFwd()
{
  ...
  //--owen
  printf("\n\n ========  Owen :  loading StFwdMip.so\n\n") ;
  gSystem->Load("StFwdMip.so") ;
            ...
            char* outfile)
{
  ...
  if (findAndRemoveOption("picodst",optionTokens)) {
    ...
    fwdTrack->SetDebug();
    //-- owen:
    /////////////fwdTrack -> setPrimaryVertexSigmaZ(50.) ;
    /////////////fwdTrack -> setPrimaryVertexSigmaXY(0.1) ;
    ...
    //-- owen
    printf("\n\n ====== Owen : adding(?) StFwdMipAnalysisMaker\n\n") ;
    StFwdMipAnalysisMaker * fwdMipAna = new StFwdMipAnalysisMaker();
    //fwdMipAna->setQuiet(false) ;
    fwdMipAna->setQuiet(true) ;
...
void mip(){
  ...
  genDst( 200, "y2023a picodst PicoVtxMode:PicoVtxDefault", "st_fwd_22355067_raw_1000028.MuDst.root", "PROD.root");
}

void mip( TString f, int n = 500){
